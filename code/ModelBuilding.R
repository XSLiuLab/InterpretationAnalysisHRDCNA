...
feature_del <- feature_pvalue %>% filter(feature_pvalue$p.signif == "ns")
...
fea_q <- fea %>% group_split(fea$feature)
for (i in 1:length(fea_q)) {
  dt <- compare_means(count ~ type, data = fea_q[[i]])
  dt$feature <- fea_q[[i]]$feature[i]
  write.table(dt, file="./supp/data/feature_qvalue.txt", append=T, row.names = F)
}

feature_qvalue <- read.table("./supp/data/feature_qvalue.txt", header = T)
feature_qvalue <- feature_qvalue %>% filter(feature_qvalue$.y. != ".y.")

# saveRDS(feature_qvalue, file = "./supp/data/feature_qvalue.rds")
