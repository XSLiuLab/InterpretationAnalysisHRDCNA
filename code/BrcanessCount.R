...
nobrca <- inner_join(TCGAalldata, nobrca, by="sample") # 8035

colnames(nobrca)[1] <- "Predictedscore"


# HRD
HRDtcga <- nobrca %>% filter(Predictedscore > 0.16) # 1274
...
final_matrix <- matrix_num[!logical_col] # 24 genes
...
mun$Relative_Count <- mun$Count/1274
mun$Type <- "HRD"
...
# HRP
HRPtcga <- nobrca %>% filter(Predictedscore < 0.16) # 6394
...
logical_col <- apply(matrix_num, 2, function(x) all(!(x %in% condition)))
final_matrix <- matrix_num[!logical_col] # 59 genes
...
mun_hrr$Relative_Count <- mun_hrr$Count/6394
mun_hrr$Type <- "HRP"
